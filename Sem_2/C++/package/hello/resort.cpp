...
void roomDetails();
int checkAlreadyUserIdExist(char* userId);
...
public:
    void createNewAccount(){
        ...
        do{
            cout<<endl<<"User ID   :  ";
            cin>>user_id;
            if (checkAlreadyUserIdExist(user_id)){
                cout<<endl<<"user ID exists Already!"<<endl;
            }
            else{
                break;
            }
        ...
        do{
            if (flag!=0){
                cout<<endl<<"Invalid Mobile NUmber ! Enter Again"<<endl;
            }
            cout<<endl<<"Mobile number   :  ";
            cin>>user_mobile;
            flag++;
        }while(!(strlen(user_mobile)==10));
...
int checkAlreadyUserIdExist(char* id){
    int flag=0;
    ...
    if (!f){
        ...
        while(f.read((char*)&u_ptr,sizeof(u_ptr))){
            if (!strcmp(u_ptr.getUserID(),id)){
                f.close();
                return 1;
            }
        }
